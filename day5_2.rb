...
class Line
    ...
    def coversRegion
        if self.isHorizontal
            ...
            toRet = Array.new
            rx = @x1 < @x2 ? [*@x1..@x2] : [*@x2..@x1].reverse
            ry = @y1 < @y2 ? [*@y1..@y2] : [*@y2..@y1].reverse
            rx.each_with_index {
                |x, i|
                y = ry[i]
                toRet.push([x,y])
...
lines.each {
    |line|
    puts "#{line.lineStr} - #{line.isDiagonal ? "diagonal" : "not diagonal"}"
}
...
blankGrid = Array.new(Line.maxx + 1) {Array.new(Line.maxy + 1, 0)}
